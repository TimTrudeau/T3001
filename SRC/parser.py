...
class Waypoint(Var):
    ...
    """
    def __init__(self, token: Token, point: dict):
        token.type = WAYPOINT
...
class Parser(object):
    ...
    def moveto_statement(self):
        ...
        if self.current_token.type == COMMA:
            self.eat(COMMA)
            varnode_s = self.factor()
        else:
            varnode_s = self.empty()
        movetoken.value = self.point(varnode_d, varnode_s)
    ...
    def waypoint_declaration(self):
        """Lexeme
        WAYPOINT_declaration : ID COLON Number(distance), Number(speed)
        """
    ...
    def point(self, factorD, factorS):
        """Lexeme
        point : (ID | factor) COMA (ID | factor)
        """
        d = factorD
        s = factorS
        return {'distance': d, 'speed': s}
